...
const baseTokens: TokenMarketData[] = [
  // Top 20 tokens with complete data
  {
    ...
    // Campos obrigatórios (novos nomes padronizados)
    currentPrice: 112717.22,
    priceChange24h: -2.27,
    priceChange7d: -2.30,
    marketCap: 2234567890000,
    totalVolume: 45678901234,
    marketCapRank: 1,
    // Campos opcionais (nomes antigos - compatibilidade)
    price: 112717.22,
    change_5m: 0.12,
    change_15m: -0.25,
    change_30m: 0.08,
  ...
  },
  {
    ...
    currentPrice: 4208.85,
    priceChange24h: -5.41,
    priceChange7d: -6.94,
    marketCap: 506789012345,
...
const generatedTokens = Array.from({ length: 90 }, (_, i) => {
    ...
    const change24h = baseChange;
    const change7d = generateRandomChange(baseChange * 1.5, 10, rank * 8);
    const marketCap = Math.max(1000000, 100000000000 / Math.pow(rank, 2));
    const volume24h = Math.max(100000, 10000000000 / Math.pow(rank, 1.8));
    ...
    return {
      ...
      change_1w: change7d,
      change_7d: change7d,
      change_30d: generateRandomChange(baseChange * 2.5, 15, rank * 9),
