...
const { publicKey, connected } = wallet;
  ...
  const handleRealDeposit = useCallback(async (amountSol: number) => {
    ...
    try {
      ...
      const signature = await depositSol(wallet, amountSol);
      
      toast.info('Confirmando depósito...');
      
      // Update balances after successful deposit
      const [newWalletBalance, newDepositedBalance] = await Promise.all([
  ...
  const handleWithdraw = useCallback(async () => {
    if (!publicKey) {
      toast.error('Conecte sua carteira para retirar');
      return;
    }
    ...
    try {
      ...
      const signature = await withdrawSol(wallet, amount);
