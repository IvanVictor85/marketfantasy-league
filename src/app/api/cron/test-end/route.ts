...
/**
 * GET /api/cron/test-end
 *
 * Handler para Vercel Cron (crons usam GET por padrão)
 * Executa a mesma lógica do POST quando chamado com CRON_SECRET
 */
export async function GET(request: NextRequest) {
  const startTime = Date.now();

  try {
    // Verificar CRON_SECRET (Vercel envia automaticamente)
    const authHeader = request.headers.get('authorization');
    const cronSecret = process.env.CRON_SECRET;

    // Se não tem auth header, retornar apenas status (para debug manual)
    if (!authHeader) {
      ...
      return NextResponse.json({
        ...
        usage: 'Vercel Cron chama automaticamente com CRON_SECRET',
    ...
    console.log('🧪 [TEST-END] Cron executado via GET - Iniciando finalizacao...');
